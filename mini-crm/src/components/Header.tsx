...
const Header = () => {
  ...
  return (
    <header className="sticky top-0 z-10 h-16 flex items-center justify-between px-6 bg-card border-b border-border">
      <div className="flex items-center bg-bg rounded-lg px-3 py-2 w-full max-w-md">
        <Search size={18} className="text-text-secondary mr-2" />
        <input
          type="text"
          placeholder="Rechercher..."
          className="bg-transparent border-none outline-none w-full text-text-primary placeholder-text-secondary"
        />
      </div>
      ...
      <div className="flex items-center space-x-4">
        <DropdownMenu
          trigger={
            <button
              className="p-2 rounded-full hover:bg-bg relative"
        ...
        >
          ...
          <ul className="py-2">
            <li className="px-4 py-2 text-sm text-text-secondary hover:bg-bg">
        ...
        <DropdownMenu
          trigger={
            <div className="flex items-center space-x-3 cursor-pointer">
              <img
                src={`https://api.dicebear.com/7.x/initials/svg?seed=${user ? encodeURIComponent(user.name) : "User"}`}
                alt="Avatar"
        ...
        >
          <div className="p-4 border-b border-border">
            <p className="font-semibold text-text-primary">{user ? user.name : "Chargement..."}</p>
            ...
            <li className="border-t border-border mt-2 pt-2">
              <button
                onClick={() => {
                  ...
                  localStorage.removeItem("user");
                  setUser(null);
                  // Optionnel: redirection après déconnexion
