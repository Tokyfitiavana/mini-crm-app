// Chat.tsx (correctif avec authToken dans la connexion Socket.IO)
...
const Chat = () => {
  ...
  useEffect(() => {
    ...
    socket.on("receive_message", ({ conversationId, message }: { conversationId: number; message: Message }) => {
      ...
      <div className="flex h-full">
        <div className="w-full md:w-1/3 border-r border-border flex flex-col">
          ...
          <div className="p-2 border-b border-border">
            <h4 className="text-sm font-semibold text-text-secondary mb-2">Utilisateurs en ligne</h4>
            {onlineUsers.map((user) => (
              <button
                key={user.id}
                onClick={() => startConversation(user.id)}
                className="w-full flex items-center gap-2 px-3 py-2 hover:bg-card rounded-md"
              >
              ...
              >
                ...
                <div className="flex-1 overflow-hidden">
                  <div className="flex justify-between items-center">
                    <h3 className="font-semibold text-text-primary truncate">{convo.user.name}</h3>
        ...
        {activeConversation && (
          <div className="hidden md:flex w-2/3 flex-col bg-bg">
            <div className="p-4 border-b border-border flex items-center gap-3">
              ...
              />
              <div>
                <h3 className="font-semibold text-text-primary">{activeConversation.user.name}</h3>
                  ...
                  >
                    ...
                    <p
                      className={`text-xs mt-1 ${
                        msg.sender === "me" ? "text-purple-200" : "text-text-secondary"
                      } text-right`}
                    >
                      {msg.timestamp}
                    </p>
