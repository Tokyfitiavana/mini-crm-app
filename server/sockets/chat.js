...
let connectedUsers = []; // [{ id, name, avatar, socketId }]
    ...
    try {
      ...
      socket.on("start_conversation", async ({ user1Id, user2Id }) => {
        ...
        const [messages] = await db.query(
          "SELECT id, sender_id, content AS text, created_at AS timestamp FROM messages WHERE conversation_id = ? ORDER BY created_at ASC",
          [conversationId]
        );
